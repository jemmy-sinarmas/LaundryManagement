...
export async function getDashboardRevenue(
  ...
  const rows = await db<RevenueRow>`
    SELECT
...
export async function getOrdersByStatus(
  ...
  const rows = await db<StatusCountRow>`
...
export async function getTop5Customers(
  ...
  const rows = await db<TopCustomerRow>`
...
export async function getDailyOrders(
  ...
  const rows = await db<DailyOrderRow>`
...
export async function getDailyRevenue(db: SqlDb, date: string): Promise<number> {
  const rows = await db<SumRow>`
...
export async function getDailyExpenses(db: SqlDb, date: string): Promise<number> {
  const rows = await db<SumRow>`
...
export async function getMonthlyRevenueByDay(
  ...
  const rows = await db<RevenueByDayRow>`
...
export async function getMonthlyRevenueByItemType(
  ...
  const rows = await db<RevenueByTypeRow>`
...
export async function getExpensesByLevelInRange(
  ...
  const rows = await db<ExpenseLevelRow>`
