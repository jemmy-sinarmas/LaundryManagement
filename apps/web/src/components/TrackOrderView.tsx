...
export default function TrackOrderView({ order, loading, notFound, refreshing, onRefresh, identifier }: Props) {
      ...
      <main className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
        <div className="w-full max-w-sm rounded-xl border bg-white p-8 text-center shadow-sm">
          ...
          <p className="mb-4 text-sm text-gray-500">
            {identifier !== 'QR code'
              ? <>Invoice <span className="font-mono">{identifier}</span> tidak ada dalam sistem kami.</>
              : 'Pesanan tidak ditemukan. QR code mungkin sudah tidak berlaku.'}
          </p>
              ...
              {ORDER_STATUSES.map((status, i) => {
                ...
                return (
                  <div key={status} className="relative flex items-start gap-3">
                    <div className={`relative z-10 flex-shrink-0 rounded-full ${done ? 'text-blue-600' : 'text-gray-300'}`}>
